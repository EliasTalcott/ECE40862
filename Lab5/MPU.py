...
class MPU:
    ...
    # Error correct gyro and accelerometer
    cycles = 0
    ...
    def acceleration(self):
        ...
        acc_x = self.__bytes_to_int(acc_x) / 16384 * 9.81 - 0.7
        acc_y = self.__bytes_to_int(acc_y) / 16384 * 9.81
        acc_z = self.__bytes_to_int(acc_z) / 16384 * 9.81 + 1
    ...
    def __update_gyro(self, timer):
        ...
        # Initialize correction values
            ...
            self.pitch_correction = -self.pitch / 20
            self.roll_correction = -self.roll / 20
            self.yaw_correction = -self.yaw / 20
            self.pitch = 0
            self.roll = 0
            self.yaw = 0
            self.cycles += 1
        else:
            self.pitch += gyro_val[1] - self.pitch_offset + self.pitch_correction
            self.roll += gyro_val[0] - self.roll_offset + self.roll_correction
            self.yaw += gyro_val[2] - self.yaw_offset + self.yaw_correction
